...
<div class="outer-ui">
  ...
  <div class="mission-box">
    ...
    <p>
      We compare average daily returns around month boundaries (first/last days)
      versus the rest of the month. Select a window and a company to explore.
    </p>
  </div>

  <div class="fx-grid">
    <div class="fx-card">
      <div class="fx-label">Window</div>
      <select id="tomWindow" class="fx-select"></select>
      <div class="fx-hint">Window = first k days + last k days (k=window/2)</div>
    </div>

    <div class="fx-card">
      <div class="fx-label">Ticker</div>
      <select id="tomTicker" class="fx-select"></select>
      <div class="fx-hint">Eligible tickers depend on min_years filter.</div>
    </div>

    <div class="fx-card">
      <div class="fx-label">View</div>
      <select id="tomView" class="fx-select">
        <option value="hist">Distribution</option>
        <option value="series">Ticker Time Series</option>
      </select>
      <div class="fx-hint">Histogram vs yearly comparison plot.</div>
    </div>
  </div>

  <div class="plots-grid">
    <div class="plot-card">
      <div class="plot-title">📊 Distribution of ToM Win-Rate (%)</div>
      <div id="tomHist" class="plot-box"></div>
    </div>

    <div class="plot-card">
      <div class="plot-title">📈 Yearly ToM vs Non-ToM (selected ticker)</div>
      <div id="tomSeries" class="plot-box"></div>
    </div>
  </div>
...
<style>
  ...
  .mission-box{
    ...
    padding:1.2rem 1.3rem;border-radius:14px;margin-bottom:1rem
  }
  .fx-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:12px;margin:12px 0}
  ...
  .fx-label{font-weight:700;margin-bottom:6px}
  .fx-select{width:100%;padding:10px 10px;border-radius:12px;border:1px solid rgba(0,0,0,.15)}
  .fx-hint{margin-top:8px;font-size:.86rem;opacity:.75}
  .plots-grid{display:grid;grid-template-columns:1fr;gap:14px;margin-top:14px}
  .plot-card{
    background:#fff;border:1px solid rgba(0,0,0,.10);
    border-radius:16px;padding:10px 12px;
    box-shadow:0 10px 28px rgba(0,0,0,.06)
  }
  .plot-title{font-weight:800;margin:6px 4px 10px}
  .plot-box{width:100%;height:520px}
</style>

<script>
(async function(){
  // JSON paths (Jekyll'de baseurl varsa çalışsın diye relative)
  const SUMMARY_URL = "{{ '/assets/data/tom_summary.json' | relative_url }}";
  const SERIES_URL  = "{{ '/assets/data/tom_series.json'  | relative_url }}";

  const summary = await fetch(SUMMARY_URL).then(r => r.json());
  const series  = await fetch(SERIES_URL).then(r => r.json());

  const windows = summary.meta.windows.map(String);

  const elWindow = document.getElementById("tomWindow");
  const elTicker = document.getElementById("tomTicker");
  const elView   = document.getElementById("tomView");

  // fill window dropdown
  elWindow.innerHTML = windows.map(w => `<option value="${w}">${w}</option>`).join("");
  elWindow.value = windows.includes("6") ? "6" : windows[0];

  function fillTickerDropdown(w){
    const tickers = summary.by_window[w].tickers;
    // Dropdown çok kalabalıksa: ilk 500 göster, geri kalanı yazıyla arama ekleriz sonra
    elTicker.innerHTML = tickers.map(t => `<option value="${t}">${t}</option>`).join("");
    elTicker.value = tickers[0] || "";
  }

  function renderHist(w){
    const vals = summary.by_window[w].values;
    ...
    const layout = {
      title: `ToM Effect Distribution (window=${w}, min_years=${summary.meta.min_years})`,
      xaxis: { title: "ToM Win-Rate (%)" },
      yaxis: { title: "Number of Companies" },
      margin: { l: 60, r: 20, t: 60, b: 60 }
    };

    Plotly.newPlot("tomHist", [trace], layout, {displayModeBar: false, responsive: true});
  }

  function renderSeries(w, t){
    const payload = (series.by_window[w] && series.by_window[w][t]) ? series.by_window[w][t] : null;

    if(!payload){
      Plotly.newPlot("tomSeries", [], {title: "No series data for this selection."}, {displayModeBar:false, responsive:true});
      return;
    }

    const tr1 = { x: payload.year, y: payload.tom,     mode: "lines+markers", name: "ToM Avg Return" };
    const tr2 = { x: payload.year, y: payload.non_tom, mode: "lines+markers", name: "Non-ToM Avg Return" };

    const layout = {
      title: `${t} — ToM vs Non-ToM (window=${w})`,
      xaxis: { title: "Year" },
      yaxis: { title: "Average Daily Return" },
      margin: { l: 70, r: 20, t: 60, b: 60 }
    };

    Plotly.newPlot("tomSeries", [tr1, tr2], layout, {displayModeBar:false, responsive:true});
  }

  function applyView(){
    const w = elWindow.value;
    const t = elTicker.value;

    // ikisini de çiziyoruz ama view'e göre birini gizleyelim:
    const view = elView.value;
    document.getElementById("tomHist").parentElement.style.display   = (view === "hist") ? "block" : "none";
    document.getElementById("tomSeries").parentElement.style.display = (view === "series") ? "block" : "none";

    if(view === "hist") renderHist(w);
    else renderSeries(w, t);
  }

  // init
  fillTickerDropdown(elWindow.value);
  renderHist(elWindow.value);
  renderSeries(elWindow.value, elTicker.value);
  applyView();
  ...
  elTicker.addEventListener("change", () => {
    renderSeries(elWindow.value, elTicker.value);
    applyView();
  });

  elView.addEventListener("change", applyView);
